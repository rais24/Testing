namespace :rubber do
  namespace :sidekiq do
    ...
    task :quiet, :roles => :sidekiq do
      cmd = <<-QUIET.gsub(/^ {8}/, '')
        if [ -d #{current_path} ]; then
          cd #{current_path} && 
            if [ -f #{current_path}/tmp/pids/sidekiq.pid ]; then
              bundle exec sidekiqctl quiet #{current_path}/tmp/pids/sidekiq.pid; 
            fi;
        fi
      QUIET
    ...
    task :start, :roles => :sidekiq do
      rsudo "cd #{current_path} ; nohup bundle exec sidekiq -e #{Rubber.env} -C #{current_path}/config/initializers/sidekiq.yml -P #{current_path}/tmp/pids/sidekiq.pid -L #{current_path}/log/sidekiq.log 2>&1 &", :as => rubber_env.app_user
